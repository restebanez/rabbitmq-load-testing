...
begin
  ...
  all_queue.subscribe(block: true, manual_ack: false) do |delivery_info, properties, body|
    ...
    if count.to_i < 3 
      ...
      print '.'
      quarantine_queues[quarantine_queue_name] ||= channel.queue(quarantine_queue_name, durable: true)
      quarantine_queues[quarantine_queue_name].publish(body, message_id: message_id)
      #channel.ack(delivery_info.delivery_tag)
    end 
  end
rescue Interrupt => _
